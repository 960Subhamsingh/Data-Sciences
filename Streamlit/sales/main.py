...
@st.cache_data
def download_data():
    dfs = pd.read_csv("C:/", sheet_name=None)
...
# Displaying the data
# In streamlit using the below  function we can render several types of data from text to markdown to dataframes. It serves like one shoe fits all. However, for making use of streamlit's full potential, it is better if we use dedicated functions for output formatting of each datatype.
